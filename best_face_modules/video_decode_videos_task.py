...
class VideoDecodeVideosTask(threading.Thread):
    ...
    def run(self):
        start_time = time.perf_counter()  # 使用perf_counter获取高精度的起始时间
        frame_interval = 1 / 21  # 每帧之间的时间间隔，21帧每秒
        count = 0
        try:
            while self.capture.isOpened():
                ...
                print("队列的长度为12:", length)

                # 控制帧率，使用perf_counter保证更高的精度
                current_time = time.perf_counter()
                expected_time = start_time + count * frame_interval
