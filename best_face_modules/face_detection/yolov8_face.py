...
import math
import os
from best_face_modules.base_model.common import softmax, resize_image
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',stream=sys.stdout)

class Yolov8Face(BaseModel):
    def __init__(self, model_path, device='cpu',**kwargs) -> None:
        super().__init__(model_path, device, **kwargs)
        self.conf_threshold = kwargs.get('conf_threshold', 0.5)#java调用调整 原值为0.5
...
if __name__ == "__main__":
    from best_face_modules.base_model.common import draw_bbox, draw_landmark
    import time
    yolo8face = Yolov8Face(model_path='E:/pythonproject/best_face/best_face_modules/face_detection/yolov8-lite-t.onnx', device='gpu')
    image = cv2.imdecode(np.fromfile('E:/pythonproject/best_face/best_face_modules/face_detection/1_Handshaking_Handshaking_1_102.jpg', dtype=np.uint8), cv2.IMREAD_COLOR)
...
if __name__ == "__main__":
    from best_face_modules.base_model.common import draw_bbox, draw_landmark
    import time
    yolo8face = Yolov8Face(model_path='/home/norco/soft/best_face/best_face_modules/face_detection/yolov8-lite-t.onnx', device='gpu')
    start_time = time.perf_counter()
    count = 0
    for file in os.listdir("/home/norco/soft/WIDER_FACE_YOLO/valid/images/"):
        count = count + 1
        path = os.path.join("/home/norco/soft/WIDER_FACE_YOLO/valid/images/", file)
        print(path)
